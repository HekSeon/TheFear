...
static BOOL						g_Use;						// TRUE:使用中  FALSE:未使用
static float					g_w, g_h;					// 幅と高さ
static XMFLOAT3					g_Pos;						// ポリゴンの座標
static int						g_TexNo;					// テクスチャ番号
FADE							g_Fade = FADE_IN;			// フェードの状態
int								g_ModeNext;					// 次のモード
XMFLOAT4						g_Color;					// フェードのカラー(アルファ値)
...
void UpdateFade(void)
{
	...
	{ // フェード処理中
		if (g_Fade == FADE_OUT)
		{ // フェードアウト処理
			g_Color.w += FADE_RATE;		// アルファ値を加算して画面を消していく
			if (g_Color.w >= 1.0f)
			{
				// 鳴っている曲を全部止める
				StopSound();
		...
		{ // フェードイン処理
			g_Color.w -= FADE_RATE;		// アルファ値を減算して画面を表示させる
			if (g_Color.w <= 0.0f)
			{
				// フェード処理終了
				g_Color.w = 0.0f;
...
void DrawFade(void)
{
	...
	// タイトルの背景を描画
	{
		...
		SetSpriteColor(g_VertexBuffer, SCREEN_WIDTH / 2, TEXTURE_WIDTH / 2, SCREEN_WIDTH, TEXTURE_WIDTH, 0.0f, 0.0f, 1.0f, 1.0f, g_Color);
